...
pub(crate) async fn create_histories_table(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    let table = format!(
        "{}.{}",
...
pub(crate) async fn drop_histories_table(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    manager
        .drop_table(
            Table::drop()
                .table((History::Schema, History::Table))
